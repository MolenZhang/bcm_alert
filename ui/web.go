...
func sendAlertByMsg(w http.ResponseWriter, r *http.Request) {
	log.Infoln("----------进入短信发送---------")
...
func SendToMobile(msg chanInfo) {
	...
	msgInfo := fmt.Sprintf(`BCM 服务报警：
监控名称:%s
详细信息:%s,%s。`, msg.aMsg.Alerts[0].Labels.AlertName,
...
func getLastValue(rec string) string {
	arr := strings.Split(rec, "_")
	var count int = 0
	for range arr {
		count++
	}
...
func updAlertParam(conf *c.Config, labInfo receiverInfo, alertType map[string]string) {
	...
	log.Infoln("before OldReceiver is :", labInfo.OldReceiver)
	log.Infoln("before NewReceiver is :", labInfo.NewReceiver)

	if strings.Contains(labInfo.OldReceiver, "-") {
		labInfo.OldReceiver = strings.Replace(labInfo.OldReceiver, "-", "_", -1)
	}

	if strings.Contains(labInfo.NewReceiver, "-") {
		labInfo.NewReceiver = strings.Replace(labInfo.NewReceiver, "-", "_", -1)
	}

	if strings.Contains(labInfo.SvcName, "-") {
		labInfo.SvcName = strings.Replace(labInfo.SvcName, "-", "_", -1)
	}

	log.Infoln("end OldReceiver is :", labInfo.OldReceiver)
	log.Infoln("end NewReceiver is :", labInfo.NewReceiver)

	splRec := getLastValue(labInfo.NewReceiver)
	//更新标签匹配信息
	for index, route := range conf.Route.Routes {
		log.Infoln("更新标签前 配置中的receiver是:", route.Receiver)
		if route.Receiver == labInfo.OldReceiver {
			conf.Route.Routes[index].Receiver = labInfo.NewReceiver
			conf.Route.Routes[index].Match[labInfo.SvcName+"_receiver"] = splRec
			break
		}
	}
	...
	for index, receiver := range conf.Receivers {
		log.Infoln("更新接收者信息前 配置中的receiver是:", receiver.Name)
